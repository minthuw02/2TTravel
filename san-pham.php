...
// Kiểm tra nếu tồn tại ID loại sản phẩm
if (isset($_GET['id'])) {
	$id = $_GET['id'];
	// Lấy thông tin loại sản phẩm từ cơ sở dữ liệu
	$type_query = "SELECT id_type, typename, slug_type FROM type_product WHERE id_type = $id";
	$type_result = mysqli_query($conn, $type_query);
	$row_type = mysqli_fetch_array($type_result);
				...
				<div class="col-lg-12 padding-30">
					...
					<div class="product_main">
						<div class="row">
							...
							$product_query = "SELECT sku_product, p.image as image, name_product, price, slug, slug_type FROM product p, type_product tp, account a WHERE p.id_type = tp.id_type AND a.id_acc = p.author AND p.id_type = $id AND flag = 1 ORDER BY date_upload DESC LIMIT $start, $limit";
